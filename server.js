...
createRouter = () => {
  ...
  router.register('GET', /^\/api\/(?<key>[-A-Za-z0-9]+)$/, (req, res) => {
    if (!state.hasOwnProperty(req.matches.key)) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({status: 'error', error: `no key ${req.matches.key} in state`}));
      return;
    }

    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify(state[req.matches.key]));
  }, error, slow);

  router.register('GET', /^\/api\/(?<key>[-A-Za-z0-9]+)\/(?<id>\d+)$/, (req, res) => {
    if (!state.hasOwnProperty(req.matches.key)) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({status: 'error', error: `no key ${req.matches.key} in state`}));
    ...
    const id = Number(req.matches.id);
    if (Number.isNaN(id)) {
      res.statusCode = 400;
      res.send(JSON.stringify({error: 'bad id'}));
      return;
    }

    // redux style mimic
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify(state[req.matches.key]?.find((o) => o.id === id)));
  }, error, slow);

  router.register('POST', /^\/api\/(?<key>[-A-Za-z0-9]+)$/, (req, res) => {
    if (!state.hasOwnProperty(req.matches.key)) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({status: 'error', error: `no key ${req.matches.key} in state`}));
    ...
    if (data.id === 0) {
      state[req.matches.key] = [...state[req.matches.key], {...data, id: Date.now()}];
      res.setHeader('Content-Type', 'application/json');
    ...
    // redux style mimic
    state[req.matches.key] = state[req.matches.key]?.map((o) => {
      if (o.id !== data.id) {
        return o;
      }
      return {...data};
  ...
  router.register('PUT', /^\/api\/(?<key>[-A-Za-z0-9]+)\/(?<id>\d+)$/, (req, res) => {
    if (!state.hasOwnProperty(req.matches.key)) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({status: 'error', error: `no key ${req.matches.key} in state`}));
  ...
  router.register('DELETE', /^\/api\/(?<key>[-A-Za-z0-9]+)\/(?<id>\d+)$/, (req, res) => {
    if (!state.hasOwnProperty(req.matches.key)) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({status: 'error', error: `no key ${req.matches.key} in state`}));
    ...
    // redux style mimic
    state[req.matches.key] = state[req.matches.key].filter((o) => o.id !== id);
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({status: 'ok'}));
  }, error, slow);

  router.register('POST', '/api/fast-payments/pay', (req, res) => {
    const data = req.body;

    // redux style mimic
    state.accounts = state.accounts.map((o) => {
      if (o.primary !== true) {
        return o;
      }
      return {...o, balance: o.balance - data.amount};
    });
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({status: 'ok'}));
  }, error, slow, json);

  router.register('GET', '/api/fast-payment-types', (req, res) => {
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify(state.fastPaymentTypes));
  });
